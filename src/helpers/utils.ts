export enum Format {
   ...
   */
  export function sqlDate(dateObj: Date = new Date(), format: Format = Format.DATETIME): string {
   ...
   * @returns {String} Formatted date
   */
  export function dateFormatIndo(date: Date, convertUTC: boolean = false): string {
    if(convertUTC) {
   ...
   */
  export function isAnyEmpty(...args: any[]): boolean {
   ...
   */
  export function isAnyEmptyObject(obj: any): boolean {
    for (let key in obj) {
